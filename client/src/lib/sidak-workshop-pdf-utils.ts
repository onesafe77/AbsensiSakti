...
interface SidakWorkshopData {
    session: SidakWorkshopSession;
    records: SidakWorkshopRecord[];
    observers: SidakWorkshopObserver[];
}

export async function generateSidakWorkshopPdf(data: SidakWorkshopData): Promise<jsPDF> {
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(14);
    pdf.text('CHECKLIST PERALATAN WORKSHOP', pageWidth / 2, yPosition + 5.5, { align: 'center' });
    pdf.setFont('helvetica', 'italic');
    pdf.setFontSize(9);
    pdf.text('Untuk memantau kondisi peralatan di workshop', pageWidth / 2, yPosition + 10, { align: 'center' });

    yPosition += 14;

    const infoData = [
        ['Tanggal', data.session.tanggal || '', 'Lokasi', data.session.lokasi || ''],
        ['Waktu', data.session.waktu || '', 'Total Alat', (data.session.totalSampel || data.records.length).toString()],
        ['Shift', data.session.shift || '', 'Departemen', data.session.departemen || '']
    ];
    ...
    // Workshop checks: namaAlat, kondisi (Baik/Rusak), kebersihan (Rapi/Kotor), sertifikasi (Valid/Expired)
    const tableHeaders = [['No', 'Nama Alat', 'Kondisi', 'Kebersihan', 'Sertifikasi', 'Keterangan']];
    const tableData = data.records.map((record, idx) => [
        (idx + 1).toString(),
        record.namaAlat || '',
        record.kondisi ? 'Baik' : 'Rusak',
        record.kebersihan ? 'Rapi' : 'Kotor',
        record.sertifikasi ? 'Valid' : 'Expired',
        record.keterangan || ''
    ]);
...
export async function downloadSidakWorkshopAsJpg(data: SidakWorkshopData, filename: string): Promise<void> {
