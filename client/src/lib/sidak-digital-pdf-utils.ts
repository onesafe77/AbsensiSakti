...
export async function generateSidakDigitalPdf(data: SidakDigitalData): Promise<jsPDF> {
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(14);
    pdf.text('OBSERVASI DISPLAY DIGITAL', pageWidth / 2, yPosition + 5.5, { align: 'center' });
    pdf.setFont('helvetica', 'italic');
    pdf.setFontSize(9);
    pdf.text('Untuk memantau kondisi display digital pada unit', pageWidth / 2, yPosition + 10, { align: 'center' });
    ...
    const tableHeaders = [['No', 'Nama - NIK', 'No Lambung', 'Tipe Unit', 'Display Ada?', 'Display Berfungsi?', 'Keterangan']];
    const tableData = data.records.map((record, idx) => [
        (idx + 1).toString(),
        record.namaNik || '',
        record.noLambung || '',
        record.tipeUnit || '',
        record.displayAda ? 'V' : 'X',
        record.displayBerfungsi ? 'V' : 'X',
        record.keterangan || ''
    ]);
