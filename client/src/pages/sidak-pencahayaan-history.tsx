...
import { ClipboardCheck, Download, Calendar, Clock, MapPin, ArrowLeft, ChevronDown, Camera, Upload, Trash2, Lightbulb, FileText, Image } from "lucide-react";
...
interface SessionWithDetails extends SidakPencahayaanSession {
    records?: SidakPencahayaanRecord[];
    observers?: SidakPencahayaanObserver[];
}

export default function SidakPencahayaanHistory() {
    ...
    const { data: sessions, isLoading } = useQuery<SessionWithDetails[]>({
        queryKey: ['/api/sidak-pencahayaan/sessions'],
    });
    ...
    const uploadPhotosMutation = useMutation({
        mutationFn: async ({ sessionId, files }: { sessionId: string; files: File[] }) => {
            ...
            const response = await fetch(`/api/sidak-pencahayaan/${sessionId}/upload-photos`, {
                method: 'POST',
                body: formData,
            });
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const response = await fetch(`/api/sidak-pencahayaan/${sessionId}/photos/${photoIndex}`, {
                method: 'DELETE',
            });
    ...
    const handleDownloadPDF = async (sessionId: string) => {
        try {
            ...
            const response = await fetch(`/api/sidak-pencahayaan/${sessionId}`);
            if (!response.ok) {
            ...
            const { generateSidakPencahayaanPdf } = await import('@/lib/sidak-pencahayaan-pdf-utils');
            const doc = await generateSidakPencahayaanPdf({
                session,
                records: records || [],
                observers: observers || []
            });
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white truncate">
                                Riwayat Sidak Pencahayaan
                            </h1>
                        </div>
                        <p className="text-gray-500 dark:text-gray-400 text-xs mt-0.5">
                            Form Pengukuran Intensitas Cahaya
                        </p>
                    </div>
                </div>
                ...
                {isLoading ? (
                    ...
                    <Card className="text-center py-10">
                        <CardContent className="pt-0">
                            ...
                            <p className="text-gray-600">Belum ada riwayat Sidak Pencahayaan</p>
                            <Link href="/workspace/sidak/pencahayaan/new">
                            ...
                            >
                                <CardContent className="p-4">
                                    <div className="flex items-start justify-between gap-2 mb-3">
                                        ...
                                        <Badge variant="outline" className="bg-amber-50 text-amber-700 text-xs flex-shrink-0">
                                            {session.totalSampel} Titik
                                        </Badge>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-gray-600 truncate">{session.penanggungJawab}</p>
                                        </div>
                                    </div>
