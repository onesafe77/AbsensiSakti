...
import { ClipboardCheck, Check, X, ArrowLeft, ArrowRight, Save, Trash2, Plus, Lightbulb } from "lucide-react";
    ...
    const renderStep1 = () => (
        <Card className="border-t-4 border-t-amber-500 shadow-lg">
            <CardHeader>
                ...
                <CardDescription>Isi detail waktu dan lokasi pengukuran pencahayaan</CardDescription>
            </CardHeader>
        ...
        <div className="space-y-6">
            <Card className="bg-amber-50 dark:bg-amber-900/20 border-2 border-amber-300">
                <CardContent className="pt-6 text-center">
                    ...
                    <p className="text-sm text-amber-700 dark:text-amber-300">Titik Diukur</p>
                </CardContent>
            </Card>

            <Card className="border-t-4 border-t-amber-500 shadow-md">
                <CardHeader>
                    <CardTitle>Input Hasil Pengukuran</CardTitle>
                    <CardDescription>Catat hasil pengukuran intensitas cahaya (Lux)</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            <Label>Titik Pengambilan <span className="text-red-500">*</span></Label>
                            <Input
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                placeholder="Contoh: Lampu TL, Matahari"
                            />
                        </div>
                        <div className="space-y-2">
                            ...
                            >
                                ...
                                <SelectContent>
                                    <SelectItem value="Penerangan Umum">Penerangan Umum</SelectItem>
                                    <SelectItem value="Penerangan Lokal">Penerangan Lokal (Setempat)</SelectItem>
                                    <SelectItem value="Darurat">Lampu Darurat</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                        <div className="space-y-2">
                            <Label>Intensitas Cahaya (Lux) <span className="text-red-500">*</span></Label>
                            <Input
                                type="number"
                                value={currentRecord.intensitasLux}
                                onChange={(e) => setCurrentRecord(prev => ({ ...prev, intensitasLux: e.target.value }))}
                                placeholder="Hasil ukur Lux Meter"
                            />
                        </div>
                        <div className="space-y-2">
                            <Label>Jarak dari Sumber</Label>
                            <Input
                                value={currentRecord.jarakDariSumber}
                                onChange={(e) => setCurrentRecord(prev => ({ ...prev, jarakDariSumber: e.target.value }))}
                                placeholder="Contoh: 2 meter"
                            />
                        </div>
                        <div className="space-y-2">
                            <Label>Kondisi Visual</Label>
                            <Select
                            ...
                            >
                                ...
                                <SelectContent>
                                    ...
                                    <SelectItem value="Silau">Silau (Glare)</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        {canAddMore ? "Tambahkan Data Titik" : "Batas Maksimal Mencapai"}
                    </Button>
                </CardContent>
            </Card>
            ...
            {draft.records.length > 0 && (
                <Card>
                    <CardHeader><CardTitle>Daftar Pengukuran ({draft.records.length})</CardTitle></CardHeader>
                    <CardContent>
                            ...
                            <table className="w-full text-sm border-collapse">
                                <thead>
                                    <tr className="bg-gray-100 dark:bg-gray-800 text-left">
                                        ...
                                        <th className="p-2 border">Titik</th>
                                        <th className="p-2 border">Kondisi Visual</th>
                                        <th className="p-2 border text-center">Lux</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {draft.records.map((rec, idx) => (
                                        <tr key={idx} className="border-b">
                                            ...
                                            <td className="p-2 border">
                                                <div className="font-bold">{rec.titikPengambilan}</div>
                                                <div className="text-xs text-gray-500">{rec.jenisPengukuran}</div>
                                            </td>
                                            <td className="p-2 border">{rec.secaraVisual}</td>
                                            <td className="p-2 border text-center font-bold text-amber-700">
                        ...
                        <div className="mt-4 flex justify-end">
                            <Button onClick={() => setDraft(prev => ({ ...prev, step: 3 }))} className="bg-amber-600 hover:bg-amber-700 text-white">
                ...
                <div className="bg-amber-600 text-white p-4 shadow-md sticky top-0 z-50">
                    <div className="container max-w-2xl mx-auto">
                        <div className="flex items-center gap-4 mb-3">
                            <Button variant="ghost" size="icon" className="hover:bg-amber-700 text-white" onClick={() => navigate("/workspace/sidak")}>
                                <ArrowLeft className="h-6 w-6" />
                            </Button>
                            <div>
                                <h1 className="text-lg font-bold">Sidak Pencahayaan</h1>
                                <p className="text-xs text-amber-100">Form Pengukuran Intensitas Cahaya</p>
                            </div>
                        </div>
                        <div className="space-y-2">
                            ...
                            <Progress value={progress} className="h-3 bg-amber-800" />
                        </div>
                    </div>
                </div>
