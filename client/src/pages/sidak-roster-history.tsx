...
import { ClipboardList, Download, Calendar, Clock, MapPin, Building2, ArrowLeft, ChevronDown, FileText, Image, Camera, Upload, Trash2, User } from "lucide-react";
...
export default function SidakRosterHistory() {
  ...
  const uploadPhotosMutation = useMutation({
    mutationFn: async ({ sessionId, files }: { sessionId: string; files: File[] }) => {
      ...
      for (const file of files) {
        ...
        const urlResponse = await fetch(`/api/sidak-roster/${sessionId}/request-upload-url`, {
          method: 'POST',
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['/api/sidak-roster'] });
  ...
  const deletePhotoMutation = useMutation({
    mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
      const response = await fetch(`/api/sidak-roster/${sessionId}/photos/${photoIndex}`, {
        method: 'DELETE',
      });
  ...
  const handleDownloadPDF = async (sessionId: string) => {
    try {
      ...
      // Download PDF
      const fileName = `Sidak_Roster_${sessionData.tanggal}_${sessionData.shift.replace(' ', '_')}.pdf`;
      pdf.save(fileName);
  ...
  const handleDownloadJPG = async (sessionId: string) => {
    try {
      ...
      const fileName = `Sidak_Roster_${sessionData.tanggal}_${sessionData.shift.replace(' ', '_')}_${Date.now()}.jpg`;
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <ClipboardList className="h-6 w-6 text-red-600 flex-shrink-0" />
                ...
                <CardContent className="p-5">
                  ...
                  <div className="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-3 mb-4 grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-1.5 text-red-600 dark:text-red-400">
                        ...
                        <span className="text-[10px] uppercase font-bold tracking-wider">PT</span>
                      </div>
                      <p className="text-sm font-medium text-gray-700 dark:text-gray-200 truncate">{session.perusahaan}</p>
                    </div>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-[calc(100vw-3rem)] sm:w-64 rounded-xl p-1 shadow-xl border-gray-200 dark:border-gray-700">
                      {(!session.activityPhotos || session.activityPhotos.length === 0) && (
                        <div className="px-3 py-2 text-xs text-amber-600 bg-amber-50 dark:bg-amber-900/20 dark:text-amber-400 border-b border-amber-100 dark:border-amber-900/30 mb-1 rounded-t-lg">
