...
import { ClipboardCheck, Download, Calendar, Clock, MapPin, ArrowLeft, ChevronDown, Camera, Upload, Trash2, Wrench, FileText, Image } from "lucide-react";
...
import type { SidakWorkshopSession, SidakWorkshopRecord, SidakWorkshopObserver } from "@shared/schema";
import { format } from "date-fns";
import { id } from "date-fns/locale";
import { useState, useRef } from "react";

interface SessionWithDetails extends SidakWorkshopSession {
    records?: SidakWorkshopRecord[];
    observers?: SidakWorkshopObserver[];
}

export default function SidakWorkshopHistory() {
    ...
    const { data: sessions, isLoading } = useQuery<SessionWithDetails[]>({
        queryKey: ['/api/sidak-workshop/sessions'],
    });
    ...
    const uploadPhotosMutation = useMutation({
        mutationFn: async ({ sessionId, files }: { sessionId: string; files: File[] }) => {
            ...
            const response = await fetch(`/api/sidak-workshop/${sessionId}/upload-photos`, {
                method: 'POST',
                body: formData,
            });
        ...
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-workshop/sessions'] });
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const response = await fetch(`/api/sidak-workshop/${sessionId}/photos/${photoIndex}`, {
                method: 'DELETE',
            });
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800">
                ...
                <div className="flex items-center gap-3 pt-2">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                            <Wrench className="h-6 w-6 text-indigo-600 flex-shrink-0" />
                            <h1 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white truncate">
                                Riwayat Sidak Workshop
                            </h1>
                        </div>
                        <p className="text-gray-500 dark:text-gray-400 text-xs mt-0.5">
                            Form Checklist Peralatan Workshop
                        </p>
                    </div>
                </div>
                ...
                {isLoading ? (
                    ...
                    <Card className="text-center py-10">
                        <CardContent className="pt-0">
                            ...
                            <p className="text-gray-600">Belum ada riwayat Sidak Workshop</p>
                            <Link href="/workspace/sidak/workshop/new">
                    ...
                    <div className="space-y-3">
                        {sessions.map((session) => (
                            <Card
                                key={session.id}
                                className="overflow-hidden border border-indigo-200 dark:border-indigo-800 shadow-sm"
                            >
                                <CardContent className="p-4">
                                    <div className="flex items-start justify-between gap-2 mb-3">
                                        ...
                                        <Badge variant="outline" className="bg-indigo-50 text-indigo-700 text-xs flex-shrink-0">
                                            {session.totalSampel} Alat
                                        </Badge>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                                        <div>
                                            ...
                                            <p className="text-gray-600 truncate">{session.departemen}</p>
                                        </div>
                                    </div>
