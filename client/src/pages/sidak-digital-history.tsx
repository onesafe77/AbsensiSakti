...
import { ClipboardCheck, Download, Calendar, Clock, MapPin, ArrowLeft, ChevronDown, Camera, Upload, Trash2, Smartphone, FileText, Image } from "lucide-react";
...
interface SessionWithDetails extends SidakDigitalSession {
    records?: SidakDigitalRecord[];
    observers?: SidakDigitalObserver[];
}

export default function SidakDigitalHistory() {
    ...
    const { data: sessions, isLoading } = useQuery<SessionWithDetails[]>({
        queryKey: ['/api/sidak-digital/sessions'],
    });
    ...
    const uploadPhotosMutation = useMutation({
        mutationFn: async ({ sessionId, files }: { sessionId: string; files: File[] }) => {
            ...
            try {
                ...
                for (const file of files) {
                    const urlResponse = await fetch(`/api/sidak-digital/${sessionId}/request-upload-url`, {
                        method: 'POST',
                    ...
                    const confirmResponse = await fetch(`/api/sidak-digital/${sessionId}/confirm-upload`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
        ...
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-digital/sessions'] });
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const response = await fetch(`/api/sidak-digital/${sessionId}/photos/${photoIndex}`, {
                method: 'DELETE',
            });
    ...
    const handleDownloadPDF = async (sessionId: string) => {
        try {
            ...
            const response = await fetch(`/api/sidak-digital/${sessionId}`);
            if (!response.ok) throw new Error('Gagal mengambil data session');
            const { session, records, observers } = await response.json();
            const { generateSidakDigitalPdf } = await import('@/lib/sidak-digital-pdf-utils');
            const doc = await generateSidakDigitalPdf({ session, records: records || [], observers: observers || [] });
            const fileName = `Sidak_Digital_${session.tanggal}_${session.shift?.replace(' ', '_') || 'shift'}.pdf`;
            doc.save(fileName);
    ...
    const handleDownloadJPG = async (sessionId: string) => {
        try {
            ...
            if (!response.ok) throw new Error('Gagal mengambil data session');
            const { session, records, observers } = await response.json();
            const { downloadSidakDigitalAsJpg } = await import('@/lib/sidak-digital-pdf-utils');
            const fileName = `Sidak_Digital_${session.tanggal}_${Date.now()}.jpg`;
            await downloadSidakDigitalAsJpg({ session, records: records || [], observers: observers || [] }, fileName);
                ...
                <div className="flex items-center gap-3 pt-2">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-500 dark:text-gray-400 text-xs mt-0.5">
                            Form Inspeksi Pengawas Digital
                        </p>
                    </div>
                </div>
