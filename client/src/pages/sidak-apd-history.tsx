...
import { ClipboardCheck, Download, Calendar, Clock, MapPin, ArrowLeft, ChevronDown, Camera, Upload, Trash2, User, HardHat, FileText, Image } from "lucide-react";
...
interface SessionWithDetails extends SidakApdSession {
    records?: SidakApdRecord[];
    observers?: SidakApdObserver[];
}

export default function SidakApdHistory() {
    ...
    const { data: sessions, isLoading } = useQuery<SessionWithDetails[]>({
        queryKey: ['/api/sidak-apd/sessions'],
        // Note: Check backend route if it is /api/sidak-apd or /api/sidak-apd/sessions
        // Based on other forms it might be just /api/sidak-apd, but Antrian used /api/sidak-antrian.
        // I will assume /api/sidak-apd returns sessions list based on common pattern?
        // Let's stick to /api/sidak-apd if that is the convention for list, OR check routes.ts
        // Actually Antrian used /api/sidak-antrian/sessions in my previous code?
        // Let me check routes.ts later. For now assume consistent naming.
    });
    ...
    const uploadPhotosMutation = useMutation({
        mutationFn: async ({ sessionId, files }: { sessionId: string; files: File[] }) => {
            ...
            const response = await fetch(`/api/sidak-apd/${sessionId}/upload-photos`, {
                method: 'POST',
                body: formData,
            });
        ...
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-apd/sessions'] });
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const response = await fetch(`/api/sidak-apd/${sessionId}/photos/${photoIndex}`, {
                method: 'DELETE',
            });
    ...
    const handleDownloadPDF = async (sessionId: string) => {
        try {
            ...
            const response = await fetch(`/api/sidak-apd/${sessionId}`);
            if (!response.ok) throw new Error('Gagal mengambil data session');
            const { session, records, observers } = await response.json();
            const { generateSidakApdPdf } = await import('@/lib/sidak-apd-pdf-utils');
            const doc = await generateSidakApdPdf({ session, records: records || [], observers: observers || [] });
            const fileName = `Sidak_APD_${session.tanggal}_${session.shift?.replace(' ', '_') || 'shift'}.pdf`;
            doc.save(fileName);
    ...
    const handleDownloadJPG = async (sessionId: string) => {
        try {
            ...
            if (!response.ok) throw new Error('Gagal mengambil data session');
            const { session, records, observers } = await response.json();
            const { downloadSidakApdAsJpg } = await import('@/lib/sidak-apd-pdf-utils');
            const fileName = `Sidak_APD_${session.tanggal}_${session.shift?.replace(' ', '_') || 'shift'}_${Date.now()}.jpg`;
            await downloadSidakApdAsJpg({ session, records: records || [], observers: observers || [] }, fileName);
                ...
                <div className="flex items-center gap-3 pt-2">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                            <HardHat className="h-6 w-6 text-purple-600 flex-shrink-0" />
                            <h1 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white truncate">
                                Riwayat Sidak APD
                            </h1>
                        </div>
                        <p className="text-gray-500 dark:text-gray-400 text-xs mt-0.5">
                            Form Pemeriksaan Alat Pelindung Diri
                        </p>
                    </div>
                </div>
                ...
                {isLoading ? (
                    ...
                    <Card className="text-center py-10">
                        <CardContent className="pt-0">
                            ...
                            <p className="text-gray-600">Belum ada riwayat Sidak APD</p>
                            <Link href="/workspace/sidak/apd/new">
                            ...
                            >
                                <CardContent className="p-4">
                                    <div className="flex items-start justify-between gap-2 mb-3">
                                        ...
                                        <Badge variant="outline" className="bg-purple-50 text-purple-700 text-xs flex-shrink-0">
                                            {session.totalSampel} Personel
                                        </Badge>
                                    </div>
