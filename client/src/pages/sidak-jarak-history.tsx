...
import { ClipboardCheck, Download, Calendar, Clock, MapPin, ArrowLeft, ChevronDown, Camera, Upload, Trash2, ArrowLeftRight, FileText, Image } from "lucide-react";
...
import type { SidakJarakSession, SidakJarakRecord, SidakJarakObserver } from "@shared/schema";
import { format } from "date-fns";
import { id } from "date-fns/locale";
import { useState, useRef } from "react";

interface SessionWithDetails extends SidakJarakSession {
    records?: SidakJarakRecord[];
    observers?: SidakJarakObserver[];
}

export default function SidakJarakHistory() {
    ...
    const { data: sessions, isLoading } = useQuery<SessionWithDetails[]>({
        queryKey: ['/api/sidak-jarak/sessions'],
    });
    ...
    const uploadPhotosMutation = useMutation({
        mutationFn: async ({ sessionId, files }: { sessionId: string; files: File[] }) => {
            ...
            for (const file of files) {
                const urlResponse = await fetch(`/api/sidak-jarak/${sessionId}/request-upload-url`, {
                    method: 'POST',
                ...
                const confirmResponse = await fetch(`/api/sidak-jarak/${sessionId}/confirm-upload`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
        ...
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-jarak/sessions'] });
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const response = await fetch(`/api/sidak-jarak/${sessionId}/photos/${photoIndex}`, {
                method: 'DELETE',
            });
    ...
    const handleDownloadPDF = async (sessionId: string) => {
        try {
            ...
            const response = await fetch(`/api/sidak-jarak/${sessionId}`);
            if (!response.ok) {
            ...
            const { generateSidakJarakPdf } = await import('@/lib/sidak-jarak-pdf-utils');
            const doc = await generateSidakJarakPdf({
                session,
                records: records || [],
                observers: observers || []
            });

            const fileName = `Sidak_Jarak_${session.tanggal}_${session.shift.replace(' ', '_')}.pdf`;
            doc.save(fileName);
    ...
    const handleDownloadJPG = async (sessionId: string) => {
        try {
            ...
            const { downloadSidakJarakAsJpg } = await import('@/lib/sidak-jarak-pdf-utils');

            const fileName = `Sidak_Jarak_${session.tanggal}_${session.shift.replace(' ', '_')}_${Date.now()}.jpg`;
            await downloadSidakJarakAsJpg({
                session,
                records: records || [],
                observers: observers || []
            }, fileName);
                ...
                <div className="flex items-center gap-3 pt-2">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                            <ArrowLeftRight className="h-6 w-6 text-red-600 flex-shrink-0" />
                            <h1 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white truncate">
                                Riwayat Sidak Jarak
                            </h1>
                        </div>
                        <p className="text-gray-500 dark:text-gray-400 text-xs mt-0.5">
                            Form Observasi Jarak Aman Unit
                        </p>
                    </div>
                </div>
