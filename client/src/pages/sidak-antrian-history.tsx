...
interface SessionWithDetails extends SidakAntrianSession {
...
export default function SidakAntrianHistory() {
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const response = await fetch(`/api/sidak-antrian/${sessionId}/photos/${photoIndex}`, {
                method: 'DELETE',
            });
    ...
    const handleDownloadPDF = async (sessionId: string) => {
        try {
            ...
            const response = await fetch(`/api/sidak-antrian/${sessionId}`);
            if (!response.ok) {
            ...
            // Import utility
            const { generateSidakAntrianPdf } = await import('@/lib/sidak-antrian-pdf-utils');
            const doc = await generateSidakAntrianPdf({
                session,
                records: records || [],
                observers: observers || []
            });
                ...
                <div className="flex items-center gap-3 pt-2">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                            <ClipboardCheck className="h-6 w-6 text-orange-600 flex-shrink-0" />
                            <h1 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white truncate">
                                Riwayat Sidak Antrian
                            </h1>
                        </div>
                        <p className="text-gray-500 dark:text-gray-400 text-xs mt-0.5">
                            Form Observasi Antrian Unit
                        </p>
                    </div>
                </div>
                ...
                {isLoading ? (
                    ...
                    <Card className="text-center py-10">
                        <CardContent className="pt-0">
                            ...
                            <p className="text-gray-600">Belum ada riwayat Sidak Antrian</p>
                            <Link href="/workspace/sidak/antrian/new">
                    ...
                    <div className="space-y-3">
                        {sessions.map((session) => (
                            <Card
                                key={session.id}
                                className="overflow-hidden border border-orange-200 dark:border-orange-800 shadow-sm"
                            >
